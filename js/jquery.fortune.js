(function ($) {
  $.fn.fortune = function (args) {
    ...
    if (args === undefined) {
      throw (new Error("You must define the options.numWins"));
    }
    ...
    var numberItem = Array.isArray(options.numWins) ? options.numWins.length : options.numWins; // Number gift of game
    
    var degItem = 360 / numberItem;
    var is_spinning = false;

    //fortune.spin = function (gift) {
    fortune.spin = function (index) {
      numWin = index;
      var deferred = $.Deferred();
      var position = -Math.floor(degItem * (numWin - 1 / 2) + randomBetween(options.separation, degItem - options.separation));
      ...
      var bounceSpin = function () {
        ...
        var curPosition = Math.floor(getRotationDegrees(fortune)),
          mod = Math.floor((curPosition + degItem * 0.5) % degItem),
          diff_position,
          position_threshold = degItem / 5,
          distance_threshold = degItem / 10;
      ...
      setTimeout(function () {
        ...
        deferred.resolve(Array.isArray(options.numWins) ? options.numWins[numWin] : numWin);
        is_spinning = false;
      }, options.duration);
